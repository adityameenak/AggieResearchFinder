...
**Status:** all 7 **Cockrell School of Engineering** departments are crawled into `faculty-ut.json`. **CNS (College of Natural Sciences) is the next wave and is genuinely harder** — there is no shared theme: math/physics/chemistry/molbio share a `/directory/<slug>` URL pattern but render differently (chemistry is a Tailwind site with *no* semantic field classes — only name/email parse cleanly), and CS uses `/people/faculty-researchers/<slug>`. Each CNS dept needs its own heuristic field extraction before it reaches engineering-level completeness. The `ut` school stays `available: false` until CNS lands (or flip it for an engineering-only launch). UT reuses TAMU's dept slugs, so no new `DEPT_DISPLAY` entries are needed.
